...
class CheckController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $check=DB::transaction(function() use($data,$request) { $check=Check::create($data+['created_by'=>$request->user()->id]); if (!empty($data['financial_document_id']) && !empty($data['linked_account_id'])) { $document=FinancialDocument::findOrFail($data['financial_document_id']); $this->service->addEntry($document,['account_id'=>$data['linked_account_id'],'payment_amount'=>$data['amount'],'payment_method_id'=>$data['payment_method_id'],'transaction_date'=>$data['issue_date'] ?? now()->toDateString(),'check_id'=>$check->id,'entry_status'=>'pending','direction'=>$data['type']==='received'?'in':'out'],$request->user()->id); } return $check; });
