...
class FinancialDocumentController extends Controller
{
    ...
    private function options(Request $request): array { $projects=$this->projects($request); $items=ProjectItem::whereIn('project_id',$projects->keys())->orderBy('title')->get(); $subitems=ProjectSubitem::with('projectItem')->whereIn('project_item_id',$items->pluck('id'))->orderBy('title')->get(); return ['accounts'=>FinancialAccount::where('is_active',true)->orderBy('name')->get(),'methods'=>PaymentMethod::where('is_active',true)->orderBy('sort_order')->get(),'categories'=>FinancialCategory::where('is_active',true)->orderBy('scope')->orderBy('sort_order')->get(),'people'=>Person::where('is_active',true)->orderBy('full_name')->get(),'projects'=>$projects,'contracts'=>Contract::whereIn('project_id',$projects->keys())->orderBy('title')->get(),'items'=>$items,'subitems'=>$subitems,'tasks'=>Task::with('projectSubitem.projectItem')->whereIn('project_subitem_id',$subitems->pluck('id'))->orderBy('title')->get()]; }
